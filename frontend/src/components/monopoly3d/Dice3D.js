...
export function animateDice3D(group, delta, elapsed) {
  ...
  const heatTarget = hovered ? 1.12 : visualStage === "rollReady" ? 0.72 : isActive ? 1 : 0.18;
  ...
  group.userData.idleLift = Math.sin(elapsed * 1.8) * 0.015;
  ...
  group.scale.setScalar(1 + group.userData.heat * 0.04 + hoverPulse);
  group.userData.glow.material.opacity = 0.14 + group.userData.heat * 0.28 + (hovered ? 0.16 : 0);
  group.userData.glow.scale.setScalar(1 + group.userData.heat * 0.14 + (hovered ? Math.sin(elapsed * 7) * 0.035 : 0));

  group.userData.dice.forEach((die, index) => {
    const wobble = Math.sin(elapsed * (hovered ? 5.4 : 2.8) + index * 0.65) * (hovered ? 0.032 : 0.014);
